...
### Note ###
#
# test with: include("ls_quad.jl"); main(Dict("random-seed" => nothing, "show-solution" => true, "runtime-limit"=>1.0, "input-file" => "---"))
#
...
function main(parsed_args)
    ...
    edge_assignments = [0.0 for i in 1:e, j in 0:1, k in 0:1]
    ...
    while time() - time_start < time_limit
        ...
        for i in 1:e, j in 0:1, k in 0:1
            fr,to = edges[i]
            qt = quadratic_terms[i][3]
            assignments[i,j+1,k+1] = variable_linear_term[fr] * (2*j-1) + variable_linear_term[to] * (2*k-1) + qt * (2*j-1) * (2*k-1)
        end
        ...
        while !stuck
            for i in 1:e
                ...
                min_assignments = NamedTuple{(:edge, :vals, :energy),Tuple{Int64,Tuple{Int64,Int64},Float64}}[]
                for val_fr in 1:2, val_to in 1:2
                    energy_delta = assignments[i, val_fr, val_to]
                    if energy_delta < min_energy
                        min_energy = energy_delta
                        min_assignments = NamedTuple{(:edge, :vals, :energy),Tuple{Int64,Tuple{Int64,Int64},Float64}}[]
                    end
                    if energy_delta <= min_energy
                        push!(min_assignments, (edge=i, vals=(2*(val_fr-1)-1, 2*(val_to-1)-1), energy=energy_delta))
                    end
                end
                ...
                var_fr, var_to = edges[assign.edge]
                val_fr, val_to = assign.vals
                assignment[var_fr] = assign.val_fr
                assignment[var_to] = assign.val_to
